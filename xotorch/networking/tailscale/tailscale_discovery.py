...
from xotorch.networking.discovery import Discovery
from xotorch.networking.peer_handle import PeerHandle
from xotorch.topology.device_capabilities import DeviceCapabilities, device_capabilities, UNKNOWN_DEVICE_CAPABILITIES
from xotorch.helpers import DEBUG, DEBUG_DISCOVERY
    ...
    while True:
      try:
        ...
        for device in active_devices.values():
          ...
          if not peer_id:
            if DEBUG_DISCOVERY >= 4: print(f"{device.device_id} does not have node attributes. skipping.")
            continue
